...
export class FolderStats {
    ...
    log(): void {
        if (DEBUG.logStats) {
            ...
            console.log(chalk.greenBright('Code coverage : '), chalk.yellowBright(`${this.coveredStatements} / ${this.totalStatements} branches ( ${this.codeCoverage} %)`), chalk.redBright(coverageWithoutCompilationErrorsMessage));
