...
export abstract class AgnosticSutMethod {
    ...
    get isVoid(): boolean {
        return this.agnosticMethod.isVoid;
    }
    ...
    async addTestSuites(): Promise<void> {
        try {
            await this.getPseudoRandomValues();
    ...
    private async getPseudoRandomValues(): Promise<void> {
        for (const inputConstraints of this.inputsConstraints) {
            if (inputConstraints.name) {
                inputConstraints.pseudoRandomValues = await ConstraintsSolverTS.resolve(inputConstraints) || [];
            }
        }
    }
    ...
    private getIndexesByPriorityOrder(): IndexesChoices[] {
        return new IndexesChoiceService().getIndexesByPriorityOrder(this.inputsConstraints.length, 10);
    }
