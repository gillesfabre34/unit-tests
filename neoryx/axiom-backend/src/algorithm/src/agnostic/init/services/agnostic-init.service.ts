...
export abstract class AgnosticInitService {
    ...
    abstract getConfigPath(projectPath: any): string;
    ...
    async start(projectPath: string, sutPath: string, algoPath: string): Promise<AgnosticSutFolder> {
        ...
        GLOBALS.algoPath = algoPath || projectPath
        GLOBALS.neoryxPath = `${projectPath}/neoryx`;
        GLOBALS.configFilePath = this.getConfigPath(projectPath);
        GLOBALS.ignoredFilesOrDirs = await IgnoreService.getIgnoredFilesOrDirs(projectPath);
        GLOBALS.start = Date.now();
        this.createProject();
        this.setUnitTestFrameworks();
        await this.initContext();
        this.initStats();
        return this.newSutFolder().generate(sutPath);
    }
    ...
    initReport(): FolderStats {
        ...
        for (const sourceFile of sourceFiles) {
            ...
            if (numberOfStatements > 0 && this.isFileToTest(sourceFile)) {
                folderStats.filesStats.push(this.newFileStats(sourceFile));
            }
        }
        return folderStats;
    }
    ...
    private newFileStats(sourceFile: any): FileStats {
        const fileStats = new FileStats(this.getFilePath(sourceFile));
    ...
    getSutProject(): Project {
        return this.getSubFolderProject(GLOBALS.sutPath);
    }
