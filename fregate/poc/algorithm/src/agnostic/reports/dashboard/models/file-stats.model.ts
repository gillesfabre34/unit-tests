...
export class FileStats {
    ...
    constructor(filePath?: string, totalStatements?: number, functionsStats: MethodStats[] = [], methodsStats: MethodStats[] = []) {
    ...
    get totalMethodsStatements(): number {
    ...
    async getMethodsArchives(): Promise<MethodArchive[]> {
        const methodsArchive: MethodArchive[] = [];
        for (const methodStats of this.methodsStats) {
            const methodCoverage: MethodCoverage = new MethodCoverage(methodStats.name, this.filePath, methodStats.numberOfBugs, methodStats.totalStatements, methodStats.numberOfCoveredStatements, methodStats.numberOfTests);
            methodsArchive.push(new MethodArchive(methodCoverage, await methodCoverage.hasRegressionInJsonHistoric()));
        }
