...
export class IndexesChoiceService {
     ...
     * @param numberOfInputsConstraints        The number of the elements of the array including the other arrays
     * @param maxPriorityOrder      Stops the process for this priority order
     */
    getIndexesByPriorityOrder(numberOfInputsConstraints: number, maxPriorityOrder: number): IndexesChoices[] {
        if (!numberOfInputsConstraints || !maxPriorityOrder) {
            return [];
        }
        let priorityOrder = 0;
        this.limitsChoices = this.initLimitsChoices(numberOfInputsConstraints);
