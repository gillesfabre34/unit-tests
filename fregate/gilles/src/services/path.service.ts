...
export class PathService {
    ...
    static getDescribePaths(describe: Describe): Path[] {
        return this.getPaths(describe.method);
    }


    static getMethodStatements(methodDeclaration: MethodDeclaration): MethodStatement[] {
        let methodStatements: MethodStatement[] = [];
        let indexBlock = 0;
        methodDeclaration.getDescendantsOfKind(SyntaxKind.Block).forEach(block => {
            const blockStatements: Statement[] = block.getStatements();
            for (let indexStatement = 0; indexStatement < blockStatements.length; indexStatement++) {
                const methodStatement = new MethodStatement();
                methodStatement.id = `${indexBlock}-${indexStatement}`;
                methodStatement.statement = blockStatements[indexStatement];
                methodStatements.push(methodStatement);
            }
            indexBlock++;
        })
    ...
    static getPaths(methodDeclaration: MethodDeclaration): Path[] {
